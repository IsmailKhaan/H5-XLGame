<template>
  <view class="bg">
    <view class="fixed_top">
      <zhnavbar
        :title="$t('transaction_record_view.transaction_record_title')"
        :lefticon="true"
      ></zhnavbar>
      ...
      <div class="bet-menus-bg" v-show="menu" @click.stop="menuFx">
        <div class="bet-menus">
          <div class="mains">
            <div
              v-for="(v, index) in mune"
              :key="index"
              @click.stop="menusFx(v, index)"
            >
              <span class="name f-c"> {{ v.name }}</span>
              <span
                :class="{ borders: index !== mune.length - 1 }"
                class="f-c"
              ></span>
            </div>
          </div>
    ...
    <view
      ...
      v-for="(v, index) in lists.rows"
      :key="index"
      v-if="datas.name == '存款' || datas.name == '取款'"
    >
      <view class="transaction_records_sbu_1 f-s">
        <view class="transaction_records_amount"
          >¥{{
            isNaN(parseFloat(v.rechargeAmt)) ? v.withdrawAmt : v.rechargeAmt
          }}</view
        >
        <view
          v-if="datas.name == '存款'"
          class="transaction_records_status f-c"
          :class="{
            status_yellow:
              $store.state.allStatus.rechargeState[v.rechargeState] == '待确认',
            status_green:
              $store.state.allStatus.rechargeState[v.rechargeState] == '成功',
            status_green:
              $store.state.allStatus.rechargeState[v.rechargeState] ==
              '确认成功',
            status_gray:
              $store.state.allStatus.rechargeState[v.rechargeState] == '提交',
            status_red:
              $store.state.allStatus.rechargeState[v.rechargeState] == '失败',
            status_red:
              $store.state.allStatus.rechargeState[v.rechargeState] ==
              '确认失败',
          }"
        ...
        <view
          ...
          :class="{
            status_yellow:
              $store.state.allStatus.withdrawState[v.withdrawState] == '待确认',
            status_green:
              $store.state.allStatus.withdrawState[v.withdrawState] == '成功',
            status_green:
              $store.state.allStatus.withdrawState[v.withdrawState] ==
              '确认成功',
            status_gray:
              $store.state.allStatus.withdrawState[v.withdrawState] == '提交',
            status_red:
              $store.state.allStatus.withdrawState[v.withdrawState] == '失败',
            status_red:
              $store.state.allStatus.withdrawState[v.withdrawState] ==
              '确认失败',
          }"
      ...
      <view
        ...
        @click="copyFx(v.rechargeOrder ? v.rechargeOrder : v.withdrawOrder)"
      >
        {{ $t("transaction_record_view.order_number") }}
        {{ v.rechargeOrder ? v.rechargeOrder : v.withdrawOrder }}
        <image
          class="copy"
          src="../../static/transfer/copy.png"
          mode="widthFix"
        ></image>
      </view>
    </view>
    ...
    >
      <view class="transaction_records_sbu_1 f-s">
        <view class="transaction_records_amount"
          >¥{{
            isNaN(parseFloat(v.dividendAmt)) ? v.rebateAmt : v.dividendAmt
          }}</view
        >
        <view
          v-if="datas.name == '返水'"
          ...
          :class="{
            status_gray:
              $store.state.allStatus.rebateState[v.rebateState] == '发放',
          }"
        ...
        <view
          ...
          :class="{
            status_gray:
              $store.state.allStatus.dividendState[v.dividendState] == '发放',
          }"
      ...
      <view
        class="transaction_records_sub_2 f-s"
        @click="copyFx(v.orderNo ? v.orderNo : v.orderNo)"
      >
    ...
    >
      ...
      <view class="transaction_records_sbu_1 f-s">
        ...
        <view
          ...
          :class="{
            status_yellow:
              $store.state.allStatus.transferStatus[v.transferStatus] ==
              '待确认',
            status_green:
              $store.state.allStatus.transferStatus[v.transferStatus] == '成功',
            status_green:
              $store.state.allStatus.transferStatus[v.transferStatus] ==
              '确认成功',
            status_gray:
              $store.state.allStatus.transferStatus[v.transferStatus] == '提交',
            status_red:
              $store.state.allStatus.transferStatus[v.transferStatus] == '失败',
            status_red:
              $store.state.allStatus.transferStatus[v.transferStatus] ==
              '确认失败',
          }"
    ...
    >
      <view class="transaction_records_sbu_1 f-s">
        ...
        <view
          ...
          :class="{
            status_yellow:
              $store.state.allStatus.applyState[v.applyState] == '待确认',
            status_green:
              $store.state.allStatus.applyState[v.applyState] == '成功',
            status_green:
              $store.state.allStatus.applyState[v.applyState] == '确认成功',
            status_gray:
              $store.state.allStatus.applyState[v.applyState] == '提交',
            status_red:
              $store.state.allStatus.applyState[v.applyState] == '失败',
            status_red:
              $store.state.allStatus.applyState[v.applyState] == '确认失败',
          }"
    ...
    GameNameFx(code) {
      for (let i = 0; i < this.mune.length; i++) {
        if (this.mune[i].uniqueCode == code) {
          return this.mune[i].platformName
            ? this.mune[i].platformName
            : "游戏名获取不到";
        }
      }
    },
...
.transaction_records {
  margin: 54rpx 34.1rpx 0 34.1rpx;
  padding: 0rpx 34.1rpx 0 34.1rpx;
    ...
    .mains {
      ...
      .name {
        ...
        border-bottom: 2rpx solid
          linear-gradient(270deg, #fff 0%, #db2b0f 52.6%, #fff 100%);
      }

      .borders {
        width: 108px;
        height: 2rpx;
        background: linear-gradient(270deg, #fff 0%, #db2b0f 52.6%, #fff 100%);
        margin: 0 auto;
      }
    }
  }
